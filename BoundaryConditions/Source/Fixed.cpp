...
void Fixed::configure(const std::string path, const bool debug)
{
    if (debug)
    {
        std::cout << "BOUNDARYCONDITIONS::FIXED::\n"
...
std::vector<std::vector<long long unsigned>> Fixed::updateRunningIndices(std::vector<std::vector<long long unsigned>> running_indices, 
                                                                         const unsigned &loc, const unsigned axis) const
{
    ...
    else if (axis == 2)
    {
        ...
        std::cout << "BOUNDARYCONDITION::FIXED::ERROR: Invalid axis argument in call to modifyRunningIndices." << std::endl;
...
std::vector<std::vector<const float *>> Fixed::generateScalarPointers(const std::vector<std::vector<long long unsigned>> &running_indices) const
{
...
std::vector<std::vector<const float *>> Fixed::generateVectorPointers(const std::vector<std::vector<long long unsigned>> &running_indices,
...
float *Fixed::getScalarFieldPointer(const unsigned &t_step, const unsigned &x_loc, const unsigned &y_loc, const unsigned &z_loc)
{
...
float *Fixed::getVectorFieldPointer(const unsigned &t_step, const unsigned &x_loc, const unsigned &y_loc, const unsigned &z_loc)
{
...
Fixed::Fixed(std::vector<float> &scalar_fields, std::vector<float> &vector_fields, std::vector<Analyser *> &analysers,
